...
def rename_existing_papers():
    ...
    for f in sorted(DEST_DIR.iterdir()):
        ...
        # Skip files that already have just a date prefix (YYYY-MM-DD [...)
        if re.match(r"\d{4}-\d{2}-\d{2}\s+\[", f.name):
            continue

        # Strip any old inverted sort key + date prefix
        base_name = re.sub(r"^(\d{4}-\d{2}-\d{2}\s+){1,2}", "", f.name)

        # Extract date from old prefix or fetch from arxiv
        old_date = re.search(r"(\d{4}-\d{2}-\d{2})\s+\[", f.name)
        if old_date:
